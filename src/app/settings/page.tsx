  ...
  return (
    <div className="max-w-[430px] mx-auto min-h-screen px-5 py-5">
      ...
      <div className="space-y-6">
        ...
        {/* Sync */}
        <div className="bg-card border border-card-border rounded-2xl p-5">
          <h2 className="font-semibold text-sm text-text-secondary mb-4 uppercase tracking-wider">Apple Health Sync</h2>
          <label className="block mb-1 text-text-secondary text-sm">API Key (for iOS Shortcut)</label>
          <div className="flex gap-2">
            <input
              type="text"
              value={settings.sync_api_key}
              readOnly
              className="flex-1 bg-bg border border-card-border rounded-xl px-4 py-3 text-sm text-text-muted font-mono"
            />
            <button
              onClick={() => navigator.clipboard.writeText(settings.sync_api_key)}
              className="px-4 py-3 rounded-xl bg-card border border-card-border text-text-secondary text-sm hover:text-text-primary transition"
            >
              Copy
            </button>
          </div>
